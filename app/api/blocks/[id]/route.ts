 ...
 *
 * RELEASE is the way back from BOTH hand marks: it answers `manualDuration: false` AND
 * `handPlaced: false`, so the row rejoins its run and, if it had been pinned to the
 * Friday buffer, is pulled back into Monday-Thursday on the same recomposition.
 * Resize is offered on EVERY row; a resize that would leave a job's blocks summing to
 ...
 *   that case.
 * - A MOVE onto Monday-Thursday does not pin the row. It sets the row's place in the
 *   queue, and the reflow then settles it contiguously after whatever precedes it. To
 *   nail a row to an exact time, follow the move with `action: "lock"`.
 * - A MOVE onto FRIDAY or the WEEKEND does pin it, and answers `handPlaced: true`. It
 *   keeps the exact slot it was dropped in, and the engine never recovers it — which is
 *   how the owner puts work on the colchón. Moving it back onto Mon-Thu clears the mark,
 *   and so does `action: "release"`. A row can carry `locked`, `handPlaced`, both or
 *   neither; they are independent.
 * - A MOVE onto the weekend, into the past or onto a hand-placed row lands where the
